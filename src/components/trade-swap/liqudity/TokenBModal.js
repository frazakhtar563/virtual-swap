...
import { setTokenBValues } from "../../../Redux/selectTokenASlice/selectTokenB";
...
const TokenBModal = (props) => {
  ...
  const addNewTokenInList = async () => {
    ...
    dispatch(setTokenBValues(newTokenDetail))
    getdetail()
    setShowImport(false)
    setIsNewToken(false)
    props.onHide()
                ...
                : showImport ? <>
                  ...
                  <div className='overY'>
                    {data.map((item) => {
                      return (
                        <>
                          <div className="d-flex mt-1" onClick={() => { item.coin == tokenA.coin ? props.onHide() : dispatch(setTokenBValues(item)); props.onHide() }} style={{ cursor: item.coin == tokenA.coin || tokenA.coin == 'BNB' && item.coin == 'WBNB' || item.coin == 'VTT' && tokenA.coin == 'Virtual' ? "no-drop" : "pointer" }}>
                            {
                              item.image_url ?
                                <img
                                  src={BnbLogo}
              ...
              !isNewToken ?
                <div className='overY'>
                  {data.map((item) => {
                    return (
                      <>
                        <div className="d-flex mt-1" onClick={() => { item.coin == tokenA.coin ? props.onHide() : dispatch(setTokenBValues(item)); props.onHide() }} style={{ cursor: item.coin == tokenA.coin || tokenA.coin == 'BNB' && item.coin == 'WBNB' ? "no-drop" : "pointer" }}>
                          <img
                            src={BnbLogo}
                            className="img-fluid mt-2 bnb_img"
                            alt=""
                          ...
                          <div className="list ms-2 mt-2">
                            <h5 className="mb-0">{item?.coin}</h5>
                            <p className="mb-0 chain">Binance Chain Native Token</p>
                          </div>
                        </div>
                      </>
                ...
                <div className='d-flex'>
                  <AiOutlineQuestionCircle className=' mark' />
                  <h5 className='ms-2'>{newTokenDetail?.coin}</h5>
                  {/* <p className='ms-2 mb-0 token_better'>BetterLogics</p> */}
