...
export const getWalletAddressOrConnect = async (shouldSwitchNetwork, refresh) => {
    ...
    const currentAddress = async () => {
        ...
        try {
            ...
            if (chainId == 97) {
                return { walletaddress: (await provider?.request({ method: 'eth_requestAccounts' }))[0], networkStatus: true }
            }
            else {
                return { walletaddress: (await provider?.request({ method: 'eth_requestAccounts' }))[0], networkStatus: false }
            }
